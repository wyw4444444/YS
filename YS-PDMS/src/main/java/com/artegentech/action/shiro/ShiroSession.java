 ...
 * 方式进行登录验证才需要，否则没必须使用本类。 shiro默认使用 ServletContainerSessionManager 来做 session
 * 管理，它是依赖于浏览器的 cookie 来维护 session 的,调用 storeSessionId 方法保存sesionId 到 cookie中
 * 为了支持无状态会话，我们就需要继承 DefaultWebSessionManager 自定义生成sessionId 则要实现
 * SessionIdGenerator 备注说明：
 */
public class ShiroSession extends DefaultWebSessionManager {
	...
	protected Serializable getSessionId(ServletRequest request, ServletResponse response) {
		...
		//System.out.println("authToken : " + sessionId);
		if (StringUtils.isEmpty(sessionId)) {
			// 如果没有携带id参数则按照父类的方式在cookie进行获取sessionId
			//System.out.println("authToken是空的！");
